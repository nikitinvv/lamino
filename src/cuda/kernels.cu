...
void __global__ gather(float2 *g, float2 *f, float *x, float *y, float *z, int m0, int m1, int m2, 
  float mu0, float mu1, float mu2, int n0, int n1, int n2, int det,  int ntheta, dir direction) {                        
  int tx = blockDim.x * blockIdx.x + threadIdx.x;
  int ty = blockDim.y * blockIdx.y + threadIdx.y;
  int tz = blockDim.z * blockIdx.z + threadIdx.z;
  ...
  float2 g0;
  float x0 = x[tx + ty * det + tz * det * det];
  float y0 = y[tx + ty * det + tz * det * det];
  float z0 = z[tx + ty * det + tz * det * det];
  int g_ind = (
    + tx
    + ty * det
    + tz * det * det
  ...
  if (direction == TOMO_FWD) {
    ...
    g0.x = g[g_ind].x / sqrtf(det*det);
    g0.y = g[g_ind].y / sqrtf(det*det);
  }
  ...
  if (direction == TOMO_FWD){
    g[g_ind].x = g0.x / sqrtf(det*det);
    g[g_ind].y = g0.y / sqrtf(det*det);
  }
}
